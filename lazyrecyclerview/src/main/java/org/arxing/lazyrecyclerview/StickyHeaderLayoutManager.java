...
public class StickyHeaderLayoutManager extends RecyclerView.LayoutManager implements PageLoadLayoutManager {
    ...
    public interface HeaderPositionChangedCallback {
        /**
         * Called when a sections header positioning approach changes. The position can be HeaderPosition.NONE, HeaderPosition.NATURAL,
         * HeaderPosition.STICKY or HeaderPosition.TRAILING
         *
    ...
    @Override public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof SavedState) {
            ...
            Log.e(TAG,
                  "onRestoreInstanceState: invalid saved state class, expected: " + SavedState.class.getCanonicalName() + " got: " +
                          state.getClass()
                                                                                                                                          .getCanonicalName());
        }
    }
    ...
    /**
     * @param fullyVisibleOnly if true, the search will be limited to the first item not hanging off top of screen or partially obscured
     *                         by a header
     * @return the viewholder for the first visible item (not header or footer)
     */
    @Nullable public SectioningAdapter.ItemViewHolder getFirstVisibleItemViewHolder(boolean fullyVisibleOnly) {
     ...
     */
    @Nullable public SectioningAdapter.HeaderViewHolder getFirstVisibleHeaderViewHolder(boolean fullyVisibleOnly) {
        return (SectioningAdapter.HeaderViewHolder) getFirstVisibleViewHolderOfType(SectioningAdapter.TYPE_HEADER, fullyVisibleOnly);
    }

    /**
     * @param fullyVisibleOnly if true, the search will be limited to the first footer not hanging off top of screen or partially obscured
     *                         by a header
     * @return the viewholder for the first visible footer (not header or item)
     */
    @Nullable public SectioningAdapter.FooterViewHolder getFirstVisibleFooterViewHolder(boolean fullyVisibleOnly) {
        return (SectioningAdapter.FooterViewHolder) getFirstVisibleViewHolderOfType(SectioningAdapter.TYPE_FOOTER, fullyVisibleOnly);
    }

    @Nullable private SectioningAdapter.ViewHolder getFirstVisibleViewHolderOfType(int baseType, boolean fullyVisibleOnly) {
        if (getChildCount() == 0) {
            return null;
        }
    ...
    @Override public View getBottommostChildView() {
        if (getChildCount() == 0) {
            return null;
        }
        ...
        if (headerPositionsBySection.containsKey(sectionIndex)) {
            ...
            if (currentHeaderPosition != newHeaderPosition) {
                ...
                if (headerPositionChangedCallback != null) {
                    headerPositionChangedCallback.onHeaderPositionChanged(sectionIndex,
                                                                          headerView,
    ...
    @Override public int getViewAdapterPosition(View view) {
    ...
    private class SmoothScroller extends LinearSmoothScroller {
        ...
        SmoothScroller(Context context, int distanceInPixels) {
            ...
            this.duration = distanceInPixels < TARGET_SEEK_SCROLL_DISTANCE_PX ? (int) (Math.abs(distanceInPixels) * millisecondsPerPx) :
                    DEFAULT_DURATION;
        }
    ...
    private static class SavedState implements Parcelable {
        ...
        @Override public String toString() {
            return "<" + this.getClass()
                             .getCanonicalName() + " firstViewAdapterPosition: " + firstViewAdapterPosition + " firstViewTop: " +
                    firstViewTop + ">";
        }
        ...
        public static final Creator<SavedState> CREATOR = new Creator<SavedState>() {
            @Override public StickyHeaderLayoutManager.SavedState createFromParcel(Parcel in) {
                return new StickyHeaderLayoutManager.SavedState(in);
            }

            @Override public StickyHeaderLayoutManager.SavedState[] newArray(int size) {
